...
function esc(str=""){return str.toString().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");}
function fmtNum(n){if(n==null)return"0";try{return Number(n).toLocaleString("en-US");}catch{return String(n);}}
function truncate(str="",max=220){if(str.length<=max)return str;return str.slice(0,max-1)+"…";}
function fmtDate(iso){if(!iso)return"—";const d=new Date(iso);if(!+d)return"—";return d.toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"});}

async function safeJson(url,init){try{const res=await fetch(url,init);if(!res.ok)return null;return await res.json();}catch(e){console.error("fetch fail",url,e);return null;}}
...
async function fetchCoreUser(id){return await safeJson(`${USERS_BASE}/v1/users/${id}`);}
async function fetchAvatarThumb(id){
  const d=await safeJson(`${THUMB_BASE}/v1/users/avatar?userIds=${id}&size=352x352&format=Png&isCircular=false`);
  if(!d||!d.data||!d.data[0])return null;return d.data[0].imageUrl||null;
}
async function fetchCounts(id){
  ...
  return {friends:f&&f.count||0,followers:fo&&fo.count||0,following:fi&&fi.count||0};
}
async function fetchPresence(id){
  const d=await safeJson(`${PRESENCE_BASE}/v1/presence/users`,{
    method:"POST",
    headers:{"content-type":"application/json"},
...
async function fetchGames(id){
  const d=await safeJson(`${GAMES_BASE}/v2/users/${id}/games?limit=6&sortOrder=Desc`);
  if(!d||!d.data)return[];
  return d.data.map(g=>({
    id:g.id,
...
async function fetchBadges(id){
  const d=await safeJson(`${BADGES_BASE}/v1/users/${id}/badges?limit=6&sortOrder=Desc`);
  if(!d||!d.data)return[];
  return d.data.map(b=>({
    id:b.id,
    name:b.name||"Badge",
...
async function fetchGameThumbs(universeIds){
  ...
  const d=await safeJson(`${THUMB_BASE}/v1/games/icons?universeIds=${qs}&size=150x150&format=Png&isCircular=false`);
  const out={};if(!d||!d.data)return out;
...
async function fetchBadgeIcons(badgeIds){
  ...
  const d=await safeJson(`${THUMB_BASE}/v1/badges/icons?badgeIds=${qs}&size=150x150&format=Png&isCircular=false`);
  const out={};if(!d||!d.data)return out;
  d.data.forEach(t=>{out[t.targetId]=t.imageUrl;});
  return out;
}
...
function buildHtml({username,data,error}){
  ...
  const titleUser=hasUser?`${data.displayName||data.name} - Roblox profile`:"Favnc Roblox Profile Viewer";
  ...
  const gamesHtml = hasUser && data.games && data.games.length
    ? data.games.map(g=>(
      ...
      (g.iconUrl?`<img src="${esc(g.iconUrl)}" alt="${esc(g.name)}">`:`<div class="game-thumb-skel"></div>`)+
      `</div>`+
      `<div class="game-meta"><div class="game-name">${esc(g.name)}</div>`+
      `<div class="game-sub">${fmtNum(g.visits)} visits • ${fmtNum(g.playing)} playing</div></div>`+
      `</a>`
    )).join("")
  ...
  const badgesHtml = hasUser && data.badges && data.badges.length
    ? data.badges.map(b=>(
      ...
      (b.iconUrl?`<img src="${esc(b.iconUrl)}" alt="${esc(b.name)}">`:`<div class="badge-thumb-skel"></div>`)+
      `</div>`+
      `<div class="badge-meta"><div class="badge-name">${esc(b.name)}</div>`+
      `<div class="badge-sub">${esc(truncate(b.description||"—",90))}</div></div>`+
      `</div>`
    )).join("")
  ...
  return "<!DOCTYPE html><html lang=\"en\"><head>"+
    ...
    `<div id="shell">`+
      ...
      `<form id="search" autocomplete="off"><input id="search-input" type="text" placeholder="Search username…" spellcheck="false" value="${esc(username||"")}"><button id="search-btn" type="submit">View</button></form></header>`+
      `<main id="layout"><section id="left">`+
        `<div id="avatar-card" class="glass"><div id="avatar-frame">`+
        (hasUser&&data.avatarUrl?`<img id="avatar-img" src="${esc(data.avatarUrl)}" alt="Avatar" loading="lazy">`:`<img id="avatar-img" src="" alt="Avatar" style="display:none">`)+
        `<div id="avatar-skeleton"></div></div>`+
        `<div id="identity"><div id="display-name">${esc(displayName)}</div><div id="username">@${esc(actualUser)}</div>`+
        `<div id="presence-pill" class="${esc(presence.cls)}">${esc(presence.text)}</div></div></div>`+
        `<div id="about-card" class="glass"><div class="card-title">About</div>`+
        `<p id="bio">${esc(bio)}</p>`+
        `<div class="meta-row"><span class="meta-label">Joined</span><span class="meta-value" id="joined">${esc(joined)}</span></div>`+
        `<div class="meta-row"><span class="meta-label">UserId</span><span class="meta-value" id="userid">${esc(userId)}</span></div>`+
        `<div class="meta-row"><span class="meta-label">Verified</span><span class="meta-value" id="verified">${esc(verified)}</span></div></div>`+
      `</section><section id="right">`+
        ...
        `<div id="panel-overview" class="panel glass active">`+
          ...
          `</div>`+
          `<div class="info-block"><div class="info-title">Status</div><div class="info-body" id="status">${esc(statusText)}</div></div>`+
          `<div class="info-block"><div class="info-title">Blurb</div><div class="info-body" id="blurb">${esc(blurb)}</div></div>`+
        `</div>`+
...
export async function onRequest({request}){
  ...
  let username=url.searchParams.get("user")||url.searchParams.get("u")||url.searchParams.get("name")||url.searchParams.get("username")||"";
  if(!username&&url.search&&url.search.startsWith("?"))username=decodeURIComponent(url.search.slice(1));
  ...
  if(username){
    ...
    if(!lookup||!lookup.data||!lookup.data.length){
      ...
      if(!core){
        ...
        }catch(e){console.error("thumbs fail",e);}
